...
@interface TwitterXMLParser(Private)
...
static void startElementSAX(void *ctx, 
	...
	if (TAGMATCH(localname, "status") || d) {
		[p->currentMessage release];
		p->currentMessage = [[Message alloc] init];
		[p->currentIconURL release];
	...
	if (p->currentMessage) {
		...
		if (p->statusTagChild) {
			if (TAGMATCH(localname, "id")) {
				...
				NSString *s = [XMLHTTPEncoder decodeXML:currentStringValue];
				s = [s gtm_stringByUnescapingFromHTML];
				s = [TwitterXMLParser decodeHeart:s];
				p->currentMessage.text = s;
			} else if (TAGMATCH(localname, "source")) {
				p->currentMessage.source = [TwitterXMLParser decodeSource:currentStringValue];
