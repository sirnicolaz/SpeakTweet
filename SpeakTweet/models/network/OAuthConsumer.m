...
- (void)requestTokenTicket:(OAServiceTicket *)ticket didFinishWithData:(NSData *)data {
	if (ticket.didSucceed) {
		...
		[[Account sharedInstance] setWaitForOAuthCallback:YES];
...
- (void)accessTokenTicket:(OAServiceTicket *)ticket didFinishWithData:(NSData *)data {
	if (ticket.didSucceed) {
		...
		[[Account sharedInstance] setUserToken:token];
		[token release];

		NSString *screenName = [self screenNameFromHTTPResponseBody:responseBody];
		[[Account sharedInstance] setScreenName:screenName];
		[responseBody release];
		
		[(AppDelegate*)[UIApplication sharedApplication].delegate resetAllTimelinesAndCache];
	}
}
