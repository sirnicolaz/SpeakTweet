...
- (void)loadView {  
	...
	webView = [WebView sharedInstance];
...
- (void)dealloc {
	LOG(@"BrowserViewController#dealloc");
...
- (void)webView:(UIWebView *)aWebView didFailLoadWithError:(NSError *)error {
	if (error.code != -999) {
		[[Alert instance] alert:@"Browser error" withMessage:error.localizedDescription];
	}
}
...
- (void)tweetURLButtonPushed:(id)sender {
	[[TwitterPost shardInstance] updateText:[[[webView request] mainDocumentURL] absoluteString]];
	[TweetPostViewController present:self];
}
