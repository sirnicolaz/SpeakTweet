...
GTMOBJECT_SINGLETON_BOILERPLATE(HttpClientPool, sharedInstance)
...
- (Class)classFromType:(HttpClientPoolClientType)type {
	switch (type) {
		case HttpClientPoolClientType_TwitterClient:
			return [TwitterClient class];
		case HttpClientPoolClientType_IconDownloader:
			return [IconDownloader class];
		case HttpClientPoolClientType_TwitterUserClient:
			return [TwitterUserClient class];
	}
	return [NSNull class];
}
